#!/usr/bin/env python
import threading, thread, gtk, tweepy, yaml, os, errno, re, webbrowser
...
class Charry():
    def __init__(self):
        ...
        try:
            os.makedirs(self.root_dir + "/cache/images")
        ...
        # Open settings YAML
        with open(self.root_dir + "/settings.yaml", 'r') as yaml_file:
            self.settings = yaml.load(yaml_file, Loader=Loader)
    ...
    def oauth(self):
        # Pull the consumer token and token secret out of the YAML
        consumerToken = self.settings['oauth']['consumerToken']
        consumerSecret = self.settings['oauth']['consumerSecret']
        ...
        if self.api.verify_credentials():
            ...
            self.settings['oauth']['accessToken'] = auth.access_token.key
            self.settings['oauth']['accessSecret'] = auth.access_token.secret
            yaml.dump(self.settings, file(self.root_dir + "/settings.yaml", 'w'))
            # Restart
            import sys
    ...
    def load(self):
        ...
        try:
            self.settings['oauth']['accessToken']
        except KeyError:
        ...
        consumerToken = self.settings['oauth']['consumerToken']
        consumerSecret = self.settings['oauth']['consumerSecret']
        accessToken = self.settings['oauth']['accessToken']
        accessSecret = self.settings['oauth']['accessSecret']
        auth = tweepy.OAuthHandler(consumerToken, consumerSecret)
